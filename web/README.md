...
- `scatter.html` — a companion view: every movement plotted by bar count vs.
  duration, with color/type/meter filters.
...
- `index-preview.png`, `scatter-preview.png` — committed 1200×630 (@2x) Open
  Graph / Twitter-Card thumbnails referenced by each page's `og:image`, so a
  shared link shows a real preview. Regenerated by `src/og_preview.py`, which
  palette-compresses them (256 colors, no dither) to stay well under WhatsApp's
  ~300 KB preview cutoff and **fails the build** if one exceeds 250 KB.
