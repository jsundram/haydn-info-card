...
def main(outfile, annotations_pdf, color_json, datadir):
    ...
    # set a background color if desired (white otherwise)
    # NB will need to do the same for anotations_pdf!
    pdf.setFillColorRGB(*pcolors.bg)
    pdf.rect(0, 0, page.width, page.height, fill=1)
    styles = get_styles()

    add_title(pdf, '2025 San Diego Haydn Marathon', 36, page, 'AppleChancery')
